...
export default function AdminProducts() {
  ...
  const [formData, setFormData] = useState<Product>({ name: "", price: 0, cost: 0, description: "", category: "", image: "" });
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      let imageData: string = formData.image;
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      let imageData: string = editProduct.image;
